...
| **M11** | **Novelty / insights / discriminatory power** (primary) — nested deterministic probe ladder on the M09 mature substrate (`m11_probe_panels.py`), generated probe notebooks + ladder/roster artifacts, CI `--check`; **transparent** response-surface + allocation policy in later phases; **no** benchmark semantic change; final **linkage** deferred to **M12** |
...
- **M01** proved **transport** and **initial hosted-model spread**; **submission readiness** still requires family depth, defensibility, mature Kaggle evidence, and writeup packaging (see `docs/lucid.md` §6.4). **M10** delivered the **writeup evidence pack** (figures, narrative, traceability). **M11** is the **hosted-model probe frontier** (nested P12–P72 ladder, response-surface scaffolding). **M12** carries **final benchmark–task–writeup linkage** per rules (deferred from the old M11 stub).
...
- **M11** adds **deterministic probe tasks** (P12 / P24 / P48; P72 = M09 comparable) and repo-side **ladder + roster** artifacts; **does not** infer family/difficulty/component claims from aggregate M09 exports — only from probe panels or raw per-episode exports when available.
- **M11 (ingest)** — Normalized **`m11_model_response_surface`** is generated from **Kaggle leaderboard CSV** rows (`scripts/ingest_m11_platform_exports.py`). The **default committed** export includes **P72** / `lucid_m09_mature_evidence_task` evidence; **P12 / P24 / P48** task rows require **additional** exports after probe runs. Cost/latency fields remain **NA** when the CSV does not expose them.
