...
- **M09** (in flight) adds a **deterministic M09 evidence panel** and **generated** Kaggle notebook for hosted-model runs on the mature unified substrate; **platform execution and CSV artifacts** close the loop when Phase C completes (see `docs/milestones/M09/M09_KAGGLE_RUNBOOK.md`). Does **not** replace M01’s historical score ledger.
